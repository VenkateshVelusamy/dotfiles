...
-- rose-pine-moon is applied after plugins load (see the PLUGINS section);
-- habamax is only a boot fallback until then.
...
vim.opt.timeoutlen = 1000 -- leader/mapping sequence window (matches wezterm leader)
vim.opt.ttimeoutlen = 50 -- key code timeout (terminal escape seqs; keep short)
...
-- Git branch for the statusline. Prefer mini.git's buffer-local head (event-driven,
-- no subprocess); fall back to a cached `git` call only when it isn't populated yet.
local cached_branch = ""
local last_check = 0
local function git_branch()
	local mg = vim.b.minigit_summary
	local branch = mg and mg.head_name or ""
	if branch == "" then
		local now = vim.uv.now()
		if now - last_check > 5000 then -- refresh the fallback at most every 5s
	...
	if branch ~= "" then
		return " \u{e725} " .. branch .. " " -- nf-dev-git_branch
	end
	return ""
end
...
-- Resize with tmux's letters: SPC h/j/k/l (Ctrl-arrows get eaten crossing
-- wezterm->tmux->nvim). Freed up by moving the git-hunk maps to SPC g*.
vim.keymap.set("n", "<leader>h", ":vertical resize +5<CR>", { silent = true, desc = "Resize: border left" })
vim.keymap.set("n", "<leader>l", ":vertical resize -5<CR>", { silent = true, desc = "Resize: border right" })
vim.keymap.set("n", "<leader>k", ":resize +3<CR>", { silent = true, desc = "Resize: border up" })
vim.keymap.set("n", "<leader>j", ":resize -3<CR>", { silent = true, desc = "Resize: border down" })
...
-- Autosave on focus-change only (leaving the buffer/window or nvim losing
-- focus) -- not on every insert-leave, so format-on-save doesn't churn while
-- typing. Only real, named, modifiable file buffers with unsaved changes.
...
vim.pack.add({
	...
	-- On-screen label motion, buffer tabline, in-editor side-by-side git diff
...
-- wezterm's glass shows through (re-applies the earlier set_transparent groups).
...
local function get_notes_path()
  -- Never error() here: this runs at startup, so a bad path would abort the rest
  -- of init. Detect the OS via vim (portable), not /etc/os-release (absent on mac).
  if vim.fn.has("mac") == 1 then
    return vim.fn.expand("~/Documents/Notes")
  end
  local os_release = vim.fn.system("cat /etc/os-release 2>/dev/null")
  if os_release:match("Ubuntu") then
    return "/mnt/c/Users/Rad/Documents/Notes"
  end
  -- Artix and any other unix: default under HOME.
  return vim.fn.expand("~/Documents/Notes")
end
...
-- Obsidian rejects a workspace whose path is absent, so only wire it up when the
-- notes dir exists (create it to enable notes). pcall stays as a last-resort guard
-- so any other plugin failure can't abort the rest of init.
if vim.fn.isdirectory(get_notes_path()) == 1 then
	pcall(setup_obsidian)
end
...
-- surround moves off the default `s` prefix so nvim-jump can own `s`.
...
MiniIcons.mock_nvim_web_devicons() -- bufferline/diffview look for this module

-- nvim-jump: press `s`, type a match, jump to the label. Works in normal,
-- visual, and operator-pending (so `ds<label>` deletes to the target).
require("jump").setup({})
vim.keymap.set({ "n", "x", "o" }, "s", function()
	require("jump").start()
end, { desc = "Jump to a location on screen" })

-- bufferline: visual buffer tabs at the top with ordinal numbers + icons.
...
vim.keymap.set("n", "<leader>]", "<cmd>BufferLineCycleNext<CR>", { silent = true, desc = "Next buffer" })
vim.keymap.set("n", "<leader>[", "<cmd>BufferLineCyclePrev<CR>", { silent = true, desc = "Previous buffer" })
for i = 1, 9 do
...
-- diffview: side-by-side git diff inside nvim. `gv` = git-view, no clash with
-- the LSP <leader>gd/gD/gS or the mini.git <leader>h* hunk maps.
require("diffview").setup({})
vim.keymap.set("n", "<leader>gv", "<cmd>DiffviewOpen<CR>", { desc = "Diffview: working tree changes" })
...
-- Hunk maps live in the git namespace so SPC h/j/k/l stay free for resize.
vim.keymap.set("n", "<leader>ga", MiniDiff.operator, { desc = "Stage hunk (git add)" })
...
local function FloatingTerminal()
	...
	if not has_terminal then
		-- jobstart{term=true}, not the deprecated termopen (removed in a future nvim).
...
end, { desc = "Open nvim cheat sheet" })
